function [] = generatePerturbedNormalClusters( xMin, xMax, yMin, yMax,pointCount)
%initialize point arrays
pointCloud = zeros(pointCount,2);
pointCloudPerturbed = pointCloud;
%generate a random x and y pair for each point
for i = 1:pointCount
    pointCloud(i,1) = (xMax - xMin)*normrnd(rand(),rand()) + xMin; 
    pointCloud(i,2) = (yMax - yMin)*normrnd(rand(),rand()) + yMin;
